...
int32 Database::GetLSLoginInfo(char* iUsername, char* oPassword, int8* lsadmin, int* lsstatus, bool* verified, sint16* worldadmin) {
...
int32 Database::CheckEQLogin(const APPLAYER* app, char* oUsername, int8* lsadmin, int* lsstatus, bool* verified, sint16* worldadmin,int32 ip)
{
	uchar eqlogin[40];
	char errbuf[MYSQL_ERRMSG_SIZE];
	char* query = 0;
