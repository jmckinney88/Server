...
//		strcpy(worldname, in_worldname);
		snprintf(worldname, (sizeof(worldname) - sizeof(WORLD_NAME_SUFFIX)) - 1, "%s", in_worldname);
...
void LWorldList::MakeServerListPacket(APPLAYER* outapp, int8 lsadmin) {
	...
	//sl->showusercount = 0;//0xff; // this tells it to display real user count instead of "UP"
	iterator.Reset();
	while(iterator.MoreElements())
	{
		world = iterator.GetData();
