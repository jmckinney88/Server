...
{ 
	...
	{
		if(!ResendQueue.empty() && dwARSP - ResendQueue.front()->dwARQ >= 0)
		{
		packetspending--;
		_log(NET__DEBUG, "IncomingARSP recieved.");
...
void EQOldStream::QueuePacket(const EQApplicationPacket *p, bool ack_req)
{
	...
	MakeEQPacket(new EQProtocolPacket(opcode, p->pBuffer, p->size), ack_req);
	delete p;
}
